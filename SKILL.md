...
1. **从参考模板起步**:`assets/templates/chat.html`(IM 聊天页)、`assets/templates/me.html`(个人主页)、`assets/templates/public.html`(公开动态流:Tab 页头/帖子卡片/点赞评论块/操作胶囊)是从 Figma 设计稿逐层转换的标准页面。新页面**复制最接近的模板改造**,不要白手起家——模板里的页头/导航/输入条/列表/气泡写法就是标准答案。
2. **读取规范**(模板没覆盖的部分再查):
