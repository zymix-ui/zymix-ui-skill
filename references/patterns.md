...
## 列表
- 行高 ≥64px,头像 48px **squircle**(radius-lg 16 = 尺寸 1/3),标题 列表项角色(14 Semibold),副文字 Body/Sm + --foreground-muted
...
- **隐藏原生滚动条**:滚动容器 `.screen{scrollbar-width:none}` + `.screen::-webkit-scrollbar{display:none}`;需要滚动反馈时用悬浮指示条 `.scroll-ind`(绝对定位、不占布局宽、滚动时淡入,JS 按滚动容器 `scrollTop` 计算 thumb 位置并 900ms 后淡出)。
